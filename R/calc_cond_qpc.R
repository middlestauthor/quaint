...
calc_cond_qpc = function(focal_trait, cor_trait, eigen_k, test_pcs, var_pcs){
  focal_centered = (focal_trait[1:dim(eigen_k$vectors)[1]] - mean(focal_trait)) %*% eigen_k$vectors/sqrt(eigen_k$values)
  cor_centered = (cor_trait[1:dim(eigen_k$vectors)[1]] - mean(cor_trait)) %*% eigen_k$vectors/sqrt(eigen_k$values)
  ...
  mu_cond = mean(focal_trait[1:dim(eigen_k$vectors)[1]]) + (cov_a/cor_va) * (cor_trait[1:dim(eigen_k$vectors)[1]] - mean(cor_trait))
  cond_va = focal_va - (cov_a^2)/cor_va
  myQ = ((focal_trait[1:dim(eigen_k$vectors)[1]] - mu_cond) %*% eigen_k$vectors[, test_pcs])/sqrt(eigen_k$values[test_pcs] * cond_va)
