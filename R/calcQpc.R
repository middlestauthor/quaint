...
#' @param myR a value that determines how many PCs you want to use to estimate the within-population Va parameter. The default is 50
#' @export
#' @examples
#' calcQpc()

calcQpc <- function(myZ, myU, myLambdas, myPCcutoff = 0.3, myR = 50){
  ...
  myQm = sapply(1:pcm, function(n){
    var0(myCm[n])/var0(myCm[(tailCutoff-myR):tailCutoff])
  })
  myPs = sapply(1:pcm, function(x){pf(myQm[x], 1, myR, lower.tail=F)})
